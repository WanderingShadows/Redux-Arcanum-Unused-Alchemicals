...
------------MOD CODE -------------------------
        ...
        calculate = function(self,card,context)
            if context.ending_shop and not context.blueprint then
                for k, v in ipairs(G.consumeables.cards) do
                    ...
                    elseif v.ability.set == 'Alchemical' and v.key ~= "c_ReduxArcanum_stonerot" then
                        G.E_MANAGER:add_event(Event({
                            trigger = 'before',
                            delay = 0.3,
                            func = (function()
                                ...
                                v:juice_up(0.8, 0.8)
                                card:juice_up()
                                v:set_ability(G.P_CENTERS["c_ReduxArcanum_stonerot"])
                                return true
                            end
                        )}))
                    end 
    ...
    func = function(hand)
        ...
        local eight_black = 0
        for i = 1, #hand do
            if ((next(SMODS.find_card('c_tma_morph')) and SMODS.find_card('c_tma_morph')[1].ability.extra.active and  hand[i].ability.effect == "Wild Card") or hand[i]:get_id() == 14) and (hand[i]:is_suit("Spades", nil, true) or hand[i]:is_suit("Clubs", nil, true) or hand[i]:is_suit("Halberds", nil, true)) then
                ace_black = ace_black+1
            elseif ((next(SMODS.find_card('c_tma_morph')) and SMODS.find_card('c_tma_morph')[1].ability.extra.active and hand[i].ability.effect == "Wild Card") or hand[i]:get_id() == 8) and (hand[i]:is_suit("Spades", nil, true) or hand[i]:is_suit("Clubs", nil, true) or hand[i]:is_suit("Halberds", nil, true))  then
